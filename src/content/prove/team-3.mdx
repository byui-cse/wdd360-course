...
## Overview
This activity will review the tools introduced last week, and begin the process of setting up our backend server and database for our SleepOutside application.
...
The structure uses an MVC (Model, View, Controller) approach, but we don't have views in this project since it is just an API server. also you may notice that there is not a `controllers` folder, but instead is a `services` folder. We have combined our routes and controllers into one place, and instead have chosen to move the logic out of the controller and into services. This will make our code easier to test later.

### **03** Connect to MongoDB Atlas
One member of the team should connect to [MongoDB Atlas](https://www.mongodb.com/products/platform/atlas-database/getting-started) and create a new database for this project. You can call the database `sleepoutside`. On the Clusters screen click on 'Connect', then "View Full Instructions", then select "Drivers" and make sure "Node.js" is selected. It will show you some code that will look something like this (You may have to switch from "String" to "Sample Code"):
```javascript
...
>Because there are a lot of moving parts in this activity, if you look below you will see a "Hint" section. Normally you should not look at that until *after* you have worked the problem for a while. In this case you have permission to look at it now. Feel free to copy/paste the code from the "Hint" section into your `init-db.js` file to use as a starting point then continue following the instructions.
...
10. While we are at it...the provided data is in PascalCase. Let's take this opportunity to change the keys to use camelCase instead. Take a look at the provided `lowerCaseKeys` function in the "Hint" above.
11. Call the `.insertMany()` method on the product collection you created above and pass in the updated `products` array that we imported earlier as an argument.
12. Once our work is complete, call the `.close()` method on the client object to close our connection to the database.
13. Add a new script to our `package.json` file to allow us to run our script. Add the following: `"init-db": "node --env-file=.env src/database/init-db.js"` and save the file.
14. Run the script and then login to Atlas to check to make sure the collection exists with the products in it.
...
You should be able to enter the URL `http://localhost:3000/api/v1/products` in your browser and see the list of products that we imported earlier. If you do, then you are ready to move on! 

### **05** `getProductsById`

Now that we are connected to Atlas and have some data in a collection, we need to start pulling data. The work to query the database will happen in `model` files, which will then be used in the product route/controller, finally you will need to create a new service to be used in the route/controller. Create a model, service, and route to pull the product with the provided `id` from the database.

>As you implement the Model and Service you might find that they are very simple and similar. You might be thinking "This seems like extra work to create a service AND a model, why don't I just use the model function directly in my route?" There isn't much logic to be applied for this route. Later we will see more complex service functions. It will become more apparent why we broke them up.
...
The first thing we need to do is to identify the URL for our API. We could hardcode this...but this value will usually change between development and production environments. Just like we used environment files in our server, they can be useful on the frontend as well. Vite will actually load and make our environment variables available to us. One difference between .env on the back end and frontend is where we find them. On the backend we used the `process.env` object. For the frontend we will find our variables in the `import.meta.env` object.

1. Create a file called `/.env` in the 'root' directory. Add the following: `PUBLIC_SERVER_URL=http://localhost:3000/api/v1/`
...
>Each member of the team will also need to manually create their own `.env` file with the backend connection information. This file is not included in the repo and so it won't be automatically pulled down. 
...
After working with your team for the one hour activity, click here for the instructor's solution: [server](https://github.com/matkat99/sleepoutsideServer/tree/team-03/src) and [client](https://github.com/matkat99/sleepoutsideClient/tree/v2-team-03/src).
