...
9. While we are at it...the provided data is in PascalCase. Let's take this opportunity to change the keys to use camelCase instead. Take a look at the provided `lowerCaseKeys` function in the "Hint" below.
...
12. Add a new script to our `package.json` file to allow us to run our script. Add the following: `"init-db": "node --env-file=.env src/database/initDb.js"` and save the file.
