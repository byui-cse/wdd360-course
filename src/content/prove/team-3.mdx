...
6. Create a variable called `db` and set it equal to the `.db(<database>)` method on the client variable you created above.
7. We need to setup all of the collections for our project. Though there will be more later, initially we will start with one: `products`. Do some research if needed to find out how to create a collection through the Node.js client. Since we will be running this initialiation more than once, it would be a good practice to drop and re-create the collection each time before we insert the data to avoid duplication. Create a function called `seedProducts` and pass in the `db` object as an argument. 
...
8. Call the `.insertMany()` method on the productCollection you created above and pass in the updated `products` array that we imported earlier as an argument.
9. Once the insert is complete, call the `.close()` method on the client object to close our connection to the database.
10. Add a new script to our `package.json` file to allow us to run our script. Add this script: `"init-db": "node --env-file=.env src/database/initDb.js"` and save the file.
11. Run the script and then login to Atlas to check to make sure the collection exists with the products in it.
...
We are making a request from frontend Javascript from one origin (localhost:5173) to another origin (localhost:3000). CORS is a security feature in browsers that prevents websites from making requests to other domains unless they have been explicitly allowed to do so. We have not allowed this on the server.

### **07** CORS

Go back to the server code. We need to add some code to our server so it can handle requests from other origins. We can use the `cors` middleware to easily do this.
