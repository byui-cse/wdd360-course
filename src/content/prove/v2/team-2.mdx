...
3. Make our product details page scalable so that we do not need to create a new HTML file for each product.

### **03** Setup Astro Project

Open up your terminal and navigate to the root of your project. Execute the following command to initialize a new Astro project in the current directory (ie `sleepoutsideClient`):
...
Once the setup is complete, you should see a new `astro.config.mjs` file in your project root along with some new directories like `src` and `public`. The `src` directory will contain your Astro components, pages, and layouts, while the `public` directory is where you can place static assets like images.
...
We need to move some of our existing code into this new structure. Start by moving the contents of the `images` directory into the `public` directory. Next move the contents of the `js` directory into a new directory called `src/js`. Then move the contents of the `css` directory into a new directory called `src/css`. Oh and we will need the files in the `json` directory as well. Move those into a new directory called `public/json`. Finally move the contents of the original `sleepoutsideClient/src/public` directory into the `public` directory in the new Astro project.

The `pages/index.astro` file is the main entry point for your website. In Astro, each `.astro` file in the `pages` directory corresponds to a route on your website. So `index.astro` will be served at the root URL (`/`).  If you open it up you might be surprised at how small it is.
...
5. We don't want all of our pages to have the same title. Currently the title is hardcoded in the `Layout.astro` file. We can make that dynamic by using a prop. In the frontmatter section of `Layout.astro` add the following line:
