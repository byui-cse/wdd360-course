...
2. Create a `swagger.routes.mts` file in the routes folder.
3. Import Router, swagger-ui-express, and swaggerDesign.json into our `swagger.routes.mts` file.
...
8. Make sure your server is running and browse to `http://localhost:3000/api/v1/api-docs`. You should see the interactive API documentation! You will be referring back to this page for the next steps so leave it open :).
...
While we are at it. Our type and schema files in our projects are incomplete. We will update them now to contain the correct types and schemas. Copy/paste the following code into your `types.ts` file in the backend codebase.
...
>Make sure to import your new type into the `product.service.mts` file.
...
3. Some of the work will be done in `product.services` and some in `product.model` What goes where? In services we should do all the work of checking to see what parameters were sent, and validating that they are good. Ultimately we want to create a well formatted object that we can send to the model so that it can simply request the right data. The model should only make the calls we need to the database and return the data. Here is another interface you can add to `types.mts` to help guide you:
```js
...
```
4. Start in the service. We need to build the `FindProductObj` first. Use the provided interface as a guide. Declare a variable called `findProduct` and start building out the object by adding a `search`, `limit`, and `offset` property. For limit check the passed in `query` to see if one was provided, if not set it to `20`. For offset if one was not provided then default to `0`
5.  Next work on the `q` search. (The `q` parameter would contain, for example the words you might type into a search box) For now we will search for the terms in `q` on the `name` and `descriptionHtmlSimple` fields. Check to see if you got a `q` param, if you did then build out a `find` object (of type FindProductObj) that you will pass into the model method later.
6. Then check to see if a `category` was passed in. You will follow a similar process but this time you will be comparing to the `category` field. Note that you could get both `q` and `category` in the same request!
...
8. Pass your object to the `getAllProducts` function in the `product.model` next. It will return some data so make sure to store that in a variable.
9. Now move to the model. Our mongo query will look different than it did last time. We are going to break it up into multiple steps. First get a reference to the Products collection. (`const productsCollection:Collection<Product> = mongodb.getDb().collection<Product>('products');`) First get the total count of records that will match our find conditions by using `await collection.countDocuments({query object})`. Then make a `find` query with the collection with our search conditions to get the actual records.
>In the past when we made queries, we would call `toArray()` to convert our find object into an array. This is because the `find` method returns a cursor, which is a special type of object that allows you to iterate over all of the results in your collection. Most of the time we can just convert directly, but sometimes we may need to modify our results in some way before we use it. 
>
...
10. If you added it, remove the `toArray()` from the end of our query.
11. Use the fields object to limit your results. (`cursor.project(fields)`)
12. Call `toArray()` to convert your cursor into an array. 
13. Return the totalCount and any products found to the service function, then finish formatting it (with `count`, `next` and `previous` fields, along with an array of objects that contain all of the product information. See below) and return it from the service function to the route.
14. In the route method when you get the data back from the service function you should check the count, crete an error if there are no records, and then send the results on if there are records.
