...
We can easily add indexes in our setup script by using the `createIndex` method on our collection: `await productsCollection.createIndex({ name: 1 });`. Add indexes on the `products` collection on `name`, `description`, `category`, `id`. Add indexes on the `users` collection on `name`, `email`.
...
1. Install swagger-ui-express and types (`@types/swagger-ui-express`) for it.
