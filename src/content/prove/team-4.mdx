...
5. Make sure to update your mongodb query to use your find object, and limit and offset.
...
11. Format and return the data as a JSON object with `count`, `next` and `previous` fields, along with an array of objects that contain all of the product information. (you can get the count of total records with `await collection.countDocuments({query object})`)
sults.
...
```
4. Building out the pagination wrapper is something that we will need to do more than once...so it makes sense to put it in a function that we cah re-use. Add this function to `utils.mts` as well and export it:
```javascript
