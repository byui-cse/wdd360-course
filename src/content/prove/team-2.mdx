...
```
  ...
  5. If the syntax on line 12 `(data) => data` is confusing you may want to spend more time reading about arrow functions in JavaScript. You could re-write that short callback function like this using a more traditional anonymous function declaration:

    ```javascript
    function(data) { return data; }
    ```

  6. The callback on line 17 is similar...`(item) => item.Id === id`. Another way to write that could be:
  ...
  7. If you open up the `product.js` file you will see how the `productData.mjs` module is used. First note that we have to import the function we need, then we can use that function just like if it were declared locally in that file.
